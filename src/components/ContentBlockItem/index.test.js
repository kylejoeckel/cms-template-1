...
test('renders ContentBlockItem component with loaded image', () => {
  ...
  render(<ContentBlockItem content={mockContent} index={0} mobile={false} theme={{}} registerRef={() => {}} />);
  
  // Assert that the image is rendered with the correct URL
  const imageElement = screen.getByTestId('content-image');
  expect(imageElement).toHaveStyle(`backgroundImage: url(${mockContent.contentImg})`);
...
test('renders ContentBlockItem component with placeholder image', () => {
  ...
  render(<ContentBlockItem content={mockContent} index={1} mobile={true} theme={{}} registerRef={() => {}} />);
  
  // Assert that the placeholder image is rendered
  const imageElement = screen.getByTestId('content-image');
  expect(imageElement).toHaveStyle(`backgroundImage: url(/plate_of_food_silhouette.jpg)`);
