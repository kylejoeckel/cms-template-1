...
import { useLazyLoadImage } from './useLazyLoading';
...
test('should set loaded to true when image is intersecting', () => {
  ...
  result.current[1]({ isIntersecting: true, target: { dataset: { bg: imageUrl } } });
...
test('should not set loaded to true when image is not intersecting', () => {
  ...
  result.current[1]({ isIntersecting: false, target: { dataset: { bg: imageUrl } } });
...
test('should not set loaded to true when image element is not found', () => {
  ...
  result.current[1]({ isIntersecting: true, target: { dataset: { bg: 'https://example.com/other-image.jpg' } } });
